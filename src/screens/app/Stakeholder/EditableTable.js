...
const EditableTable = props => {
    ...
    const onEditCancelClick = (id) => {
        ...
        const newConnectionDrafts = connectionDrafts.filter((edit) => edit._id !== id);
    ...
    const onAddClick = (index) => {
        setConnectionDrafts([...connectionDrafts, {
            _id: 'new'+index,
            type: 'data',
            from: '',
            to: '',
    ...
    const onFieldChange = (e, field, id) => {
        ...
        newConnectoinDrafts[index] = { ...newConnectoinDrafts[index], [field]: e.target.value};
            ...
            <Card className="bg-light mt-5 px-5 py-4">
                <Card.Body>
                    ...
                    <Table striped bordered hover size="sm">
                        ...
                        <tbody>
                            {!!connectionDrafts.length && connectionDrafts.map((connection, index)=>(
                                (editList.includes(connection._id) || (connection._id.includes('new')))
                                    ?   <tr key={connection._id}>
                                            <td className="text-center">
                                                <ConfirmButton onClick={() => onConfirmClick(connection._id)} />
                                                <CancelButton onClick={() => onEditCancelClick(connection._id)} />
                                            </td>
                                            <td>#{index + 1}</td>
                                            <td>
                                                <select
                                                    ...
                                                    onChange={(e) => onFieldChange(e, 'type', connection._id)}
                                                >
                                            ...
                                            <td>
                                                <select
                                                    ...
                                                    onChange={(e) => onFieldChange(e, 'from', connection._id)}
                                                >   
                                                    <option></option>
                                                    {stakeholders.map((sh) => (
                                                        <option key={sh._id} value={sh._id}>{sh.name}</option>
                                                    ))}
                                                </select>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                <Form.Control 
                                                    ...
                                                    placeholder="Note"
                                                    onChange={(e) => onFieldChange(e, 'note', connection._id)}
                                                    size="sm"
                                                />
                                            </td>
                                        </tr>
                                    : <tr key={connection._id}>
                                            <td className="text-center">                                   
                                                ...
                                                <DeleteButton onClick={() => onDeleteClick(connection._id)}  />
                                            </td>
                                            <td>#{index + 1}</td>
                    ...
                    <Button variant="primary" className="pull-right" onClick={() => onAddClick(connectionDrafts.length)} >+ADD</Button>
                    <Button variant="info" className="pull-right block mb-3 mr-3" onClick={handleExportCSV}>
